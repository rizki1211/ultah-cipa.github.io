const targetDate = new Date("June 5, 2025 00:00:01").getTime();
...
window.addEventListener("load", () => {
  const now = new Date().getTime();
  if (now >= targetDate) {
    initPage(); // Jalankan langsung jika sudah melewati tanggal target
  } else {
    showCountdown(); // Tampilkan countdown jika belum waktunya
  }
});
